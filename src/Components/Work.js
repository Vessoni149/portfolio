...
function Work({ position, company, location, type, duration, description }) {
  ...
  return (
    <article className="pt-8 border-b-2 border-dark-content pb-5 dark:border-light-content border-opacity-20 dark:border-opacity-20">
      <div className="flex justify-between items-center"  onClick={toggleMenu}>
        <h1 className="text-content md:text-lg lg:text-xl hover:cursor-pointer">{position} ▼</h1>
      ...
      {showMenu && (
        <div className="mt-2">
          <p className="text-content text-xs md:text-sm font-normal pl-1 min-w-fit">{description}</p>
        </div>
      )}
    </article>
  );
